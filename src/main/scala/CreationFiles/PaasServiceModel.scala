...
for the for Paas Service Simulation.

Each simulation will be passing the Config name as an parameter to this class.

In SAAS the Customer will only have access over the number of cloudlets and their configuration.
All the rest of parameters are configured by Provider.

*/

class PaasServiceModel(var simName: String) {
  //Created a logger for this class.
  val logger = CreateLogger(classOf[PaasServiceModel])
  logger.info("Generated instance of PaasServiceModel class to create the required resources. ")
  ...
  */
  ...
  def createPeList(hostnum: Int): List[PeSimple] = {
    ...
    // For PAAS, the provider will control over the following parameter
  ...
  def createHost(hostnum: Int, vmScheduler: VmScheduler): Host ={
    ...
    For PAAS, Cloud Provider will have access to this configuration. 
  ...
  def createVMList(cloudletScheduler: CloudletScheduler): List[Vm] ={
   ...
   For PAAS, Customer will have access to this configuration
